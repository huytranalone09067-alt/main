...
local PathfindingService = game:GetService("PathfindingService") -- Thêm Service tìm đường
...
local Tabs = {
    ...
    Library = Window:AddTab({ Title = "Library", Icon = "book" }),
...
Tabs.Player:AddToggle("TPWalk", { Title = "Enable TPWalk", Default = false, Callback = function(Value) tpWalkOn = Value end })
Tabs.Player:AddSlider("TPWalkSpeed", { Title = "TPWalk Speed", Default = 0.5, Min = 0.1, Max = 5, Rounding = 1, Callback = function(Value) tpWalkSpeed = Value end })
...
Tabs.Player:AddKeybind("FreecamBind", { Title = "Freecam Keybind", Mode = "Toggle", Default = "F", Callback = function() Options.Freecam:SetValue(not Options.Freecam.Value) end })
Tabs.Player:AddSlider("FreecamSpeed", { Title = "Freecam Speed", Default = 1.5, Min = 0.1, Max = 5, Rounding = 1, Callback = function(Value) freecamSpeed = Value end })
...
Tabs.Player:AddButton({ Title = "Add Gold (Visual)", Callback = function()
...
Tabs.Visuals:AddSlider("BrightnessSlider", { Title = "Fullbright Intensity", Default = 2, Min = 1, Max = 10, Rounding = 1, Callback = function(Value)
...
Tabs.ESP:AddToggle("DoorESP", { Title = "Smart Door ESP (Chỉ cửa đúng)", Default = false }):AddColorpicker("FillColor", { Title = "Door Color", Default = Color3.fromRGB(0, 255, 0) })
Tabs.ESP:AddToggle("EntityESP", { Title = "Entity Highlight", Default = false }):AddColorpicker("EntityColor", { Title = "Entity Color", Default = Color3.fromRGB(255, 0, 0) })
Tabs.ESP:AddToggle("ObjESP", { Title = "Objective ESP", Default = false }):AddColorpicker("ObjColor", { Title = "Objective Color", Default = Color3.fromRGB(0, 0, 255) })
Tabs.ESP:AddToggle("GoldPileESP", { Title = "Gold Highlight", Default = false }):AddColorpicker("GoldColor", { Title = "Gold Color", Default = Color3.fromRGB(255, 225, 0) })
Tabs.ESP:AddToggle("ItemESP", { Title = "Item Highlight", Default = false }):AddColorpicker("ItemColor", { Title = "Item Color", Default = Color3.fromRGB(0, 255, 255) })
Tabs.ESP:AddToggle("HidingESP", { Title = "Hiding ESP", Default = false }):AddColorpicker("HideCol", { Title = "Hiding Color", Default = Color3.fromRGB(100, 100, 100) })
...
-- LIBRARY TAB
--------------------------------------------------------------------------------
Tabs.Library:AddSection("Library Features")
Tabs.Library:AddToggle("LibraryBookESP", { Title = "ESP LiveHintBook", Default = false }):AddColorpicker("LibBookColor", { Title = "Book Color", Default = Color3.fromRGB(255, 0, 255) })
Tabs.Library:AddToggle("ComingSoon1", { Title = "Coming Soon...", Default = false })
Tabs.Library:AddToggle("ComingSoon2", { Title = "Coming Soon...", Default = false })

--------------------------------------------------------------------------------
-- DEMO TAB (MINIGAMES, SOLVERS & AUTO PLAY 1-50)
--------------------------------------------------------------------------------
Tabs.Demo:AddSection("Auto Play (Phòng 1 - 50)")

local evading = false -- Biến toàn cục để check xem có đang trốn không

Tabs.Demo:AddToggle("AutoWalk150", { 
    Title = "Auto Đi Bằng Pathfind (1-50)", 
    Default = false,
    Description = "Tự động tìm đường qua cửa, nhặt chìa khóa, gạt cần."
})

Tabs.Demo:AddToggle("AutoHide", { 
    Title = "Auto Trốn Tủ (Rush/Ambush)", 
    Default = false,
    Description = "Tự chui vào tủ/gầm giường gần nhất khi có Rush/Ambush."
})

Tabs.Demo:AddToggle("AutoScreech", { 
    Title = "Auto Nhìn Screech", 
    Default = false,
    Description = "Tự động xoay camera nhìn Screech trong phòng tối."
})

Tabs.Demo:AddSection("Minigames & Solvers")

Tabs.Demo:AddToggle("AutoHeartbeat", { 
    Title = "Auto Heartbeat (Tự Win Tim)", 
    Default = false,
    Description = "Tự động hoàn thành minigame nhịp tim khi trốn tủ."
})

Tabs.Demo:AddButton({
    Title = "Giải Mã Sách (Phòng 50)",
    Description = "Lấy Pass từ giấy và sách (cần nhặt giấy mật mã trước)",
    Callback = function()
        ...
        if not paper then
            Fluent:Notify({Title = "Lỗi", Content = "Bạn cần nhặt tờ giấy mật mã trên bàn trước!", Duration = 3})
            return
        end
        Fluent:Notify({Title = "Auto Solver", Content = "Đang phân tích mã... (Chức năng đang được hoàn thiện)", Duration = 3})
    end
})

Tabs.Demo:AddToggle("AutoBreaker", { 
    Title = "Auto Breaker Box (Phòng 100)", 
    Default = false,
    Description = "Tự động chơi minigame cầu dao điện (Chưa hỗ trợ hoàn toàn)"
})
...
-- Hàm Pathfind tìm đường đi hỗ trợ Auto 1-50
local function PathfindTo(targetPos)
    ...
    if success and path.Status == Enum.PathStatus.Success then
        ...
        for _, waypoint in ipairs(waypoints) do
            if not Options.AutoWalk150.Value or evading then break end -- Dừng nếu tắt Auto hoặc đang bận trốn
            if waypoint.Action == Enum.PathWaypointAction.Jump then hum.Jump = true end
            hum:MoveTo(waypoint.Position)
            -- Đợi di chuyển tới điểm đó (timeout 1.5s để tránh bị kẹt)
            local moveSuccess = hum.MoveToFinished:Wait(1.5)
        end
        return true
    end
    return false
...
-- Vòng lặp Auto Đi (1-50)
task.spawn(function()
    while task.wait(0.5) do
        if Options.AutoWalk150 and Options.AutoWalk150.Value and not evading then
            local currentRoomNum = LocalPlayer:GetAttribute("CurrentRoom")
            local rooms = workspace:FindFirstChild("CurrentRooms")
            
            if rooms and currentRoomNum then
                local room = rooms:FindFirstChild(tostring(currentRoomNum))
                if room then
                    local door = room:FindFirstChild("Door")
                    local locked = door and door:FindFirstChild("Lock")
                    
                    -- Tìm chìa khóa nếu cửa khóa
                    if locked then
                        local key = room:FindFirstChild("KeyObtain", true)
                        if key and key.PrimaryPart then
                            PathfindTo(key.PrimaryPart.Position)
                            task.wait(0.2)
                    ...
                    -- Tìm cần gạt (nếu phòng yêu cầu kéo cần mở cổng)
                    local lever = room:FindFirstChild("LeverForGate", true)
                    if lever and lever.PrimaryPart then
                        PathfindTo(lever.PrimaryPart.Position)
                        task.wait(0.2)
                        local prompt = lever:FindFirstChildWhichIsA("ProximityPrompt", true)
                        if prompt then fireproximityprompt(prompt) end
                    end

                    -- Pathfind đi đến cửa tiếp theo
                    if door and door:FindFirstChild("Door") then
                        PathfindTo(door.Door.Position)
                        task.wait(0.2)
                        local prompt = door.Door:FindFirstChildWhichIsA("ProximityPrompt", true) or door:FindFirstChildWhichIsA("ProximityPrompt", true)
                        if prompt then fireproximityprompt(prompt) end
                    end
                end
            end
...
workspace.CurrentCamera.ChildAdded:Connect(function(child)
    if Options.AutoScreech and Options.AutoScreech.Value and child.Name == "Screech" then
        task.wait(0.05)
        if child.PrimaryPart then
            Camera.CFrame = CFrame.lookAt(Camera.CFrame.Position, child.PrimaryPart.Position)
        end
    end
end)

-- Logic Collect Aura, Heartbeat
task.spawn(function()
    while task.wait(0.1) do
        ...
        -- Auto Heartbeat
        if Options.AutoHeartbeat and Options.AutoHeartbeat.Value then
            pcall(function()
                local gui = LocalPlayer.PlayerGui:FindFirstChild("MainUI")
                if gui then
                    local minigame = gui:FindFirstChild("Initiator") and gui.Initiator:FindFirstChild("Main_Game") and gui.Initiator.Main_Game:FindFirstChild("RemoteListener") and gui.Initiator.Main_Game.RemoteListener.Modules:FindFirstChild("Minigame_Heartbeat")
                    -- Bypass Heartbeat logic sẽ thực thi ở đây
                end
            end)
        end
    end
...
--------------------------------------------------------------------------------
-- INTERNAL ESP LOGIC & AUTO HIDE EVENT
--------------------------------------------------------------------------------
local activeESPs = {}

local EntityNames = {"RushMoving", "AmbushMoving", "FigureRig", "SeekMoving", "Eyes", "Screech", "A-60", "A-120", "A-90", "HaltRoom", "JeffTheKiller", "Rush", "Ambush", "Figure", "Seek"}
local ObjList = {"LibraryHintPaper", "KeyObtain", "LeverForGate", "LiveBreakerPolePickup", "ElectricalKeyObtain"}
local ItemList = {"Crucifix", "SkeletonKey", "Flashlight", "Lighter", "Lockpick", "Vitamins", "Battery"}
local HSList = {"Locker_Large", "Toolshed", "Wardrobe", "Bed", "Rooms_Locker"}
...
local function CreateESP(obj, name, type)
    ...
    local attachObj = obj
    if obj:IsA("Model") and obj.PrimaryPart then attachObj = obj.PrimaryPart end
...
workspace.ChildAdded:Connect(function(child)
    ...
    if child.Name == "RushMoving" or child.Name == "AmbushMoving" then
        if Options.AutoHide and Options.AutoHide.Value then
            evading = true
            local char = LocalPlayer.Character
            local root = char and char:FindFirstChild("HumanoidRootPart")
            if root then
                -- Tìm tủ gần nhất
                local closestHide, dist = nil, math.huge
                local rooms = workspace:FindFirstChild("CurrentRooms")
                if rooms then
                    local curRoom = rooms:FindFirstChild(tostring(LocalPlayer:GetAttribute("CurrentRoom")))
                    if curRoom then
                        for _, v in pairs(curRoom:GetDescendants()) do
                            if v.Name == "Locker_Large" or v.Name == "Bed" then
                                local hidePos = v:IsA("Model") and v.PrimaryPart.Position or v.Position
                                local d = (root.Position - hidePos).Magnitude
                                if d < dist then dist = d; closestHide = v end
                            end
                        end
                    end
                end
                -- Trốn
                if closestHide then
                    PathfindTo(closestHide:IsA("Model") and closestHide.PrimaryPart.Position or closestHide.Position)
                    task.wait(0.1)
                    local prompt = closestHide:FindFirstChildWhichIsA("ProximityPrompt", true)
                    if prompt then fireproximityprompt(prompt) end
                end
            end
        end
    ...
    if entName then 
        ...
        if Options.NotifyEntity and Options.NotifyEntity.Value and not string.find(cleanName, "Figure") then
            Fluent:Notify({ Title = "⚠️ Cảnh báo Entity", Content = cleanName .. " vừa xuất hiện!", Duration = 4 })
        end
    end
end)
...
workspace.ChildRemoved:Connect(function(child)
    ...
    if child.Name == "RushMoving" or child.Name == "AmbushMoving" then
        evading = false
        local char = LocalPlayer.Character
        -- Tự chui ra khỏi tủ (giả lập phím E)
...
task.spawn(function()
    while true do
        ...
        if rooms then
            for _, room in pairs(rooms:GetChildren()) do
                ...
                for _, child in pairs(room:GetDescendants()) do
                    ...
                    elseif Options.LibraryBookESP.Value and n == "LiveHintBook" then
                        CreateESP(child, "Book", "LibraryBook")
                    elseif Options.DoorESP.Value and n == "Door" then
                        if roomNum == currentRoomNumber then CreateESP(child, "Real Door [" .. (currentRoomNumber + 1) .. "]", "Door") end
        ...
        for _, ent in pairs(workspace:GetChildren()) do
            ...
            if entName then CreateESP(ent, entName:gsub("Moving", ""):gsub("Rig", ""):gsub("TheKiller", ""), "Entity") end
        end
        task.wait(1)
    end
end)
...
local function UpdateESP()
    ...
    for obj, data in pairs(activeESPs) do
        ...
        local isEnabled = (data.Type == "Door" and Options.DoorESP.Value) or
                          ...
                          (data.Type == "LibraryBook" and Options.LibraryBookESP.Value)

        if not obj or not obj.Parent or not isEnabled then Cleanup(obj, data); continue end
        ...
        if onScreen then
            ...
            elseif data.Type == "LibraryBook" then color = Options.LibBookColor.Value end
            
            if Options.RainbowESP.Value then color = rainbow end

            if data.Type == "Door" then
                if not data.Box then data.Box = Instance.new("BoxHandleAdornment", obj); data.Box.AlwaysOnTop = true; data.Box.Adornee = obj; data.Box.ZIndex = 5 end
                data.Box.Visible = true; data.Box.Size = obj:IsA("BasePart") and obj.Size or Vector3.new(4, 7, 1); data.Box.Color3 = color; data.Box.Transparency = 0.7
            else
                if not data.Highlight then data.Highlight = Instance.new("Highlight", obj) end
                data.Highlight.Adornee = attachObj; data.Highlight.Enabled = true; data.Highlight.FillColor = color; data.Highlight.OutlineColor = color; data.Highlight.FillTransparency = 0.5
            end
            ...
            data.Bill.Enabled = Options.EnableText.Value; data.Bill.Adornee = attachObj
            local lbl = data.Bill:FindFirstChild("L")
            if lbl then lbl.TextColor3 = color; lbl.TextSize = Options.TextSize.Value; lbl.Text = data.Name .. "\n[" .. math.floor(dist) .. "]" end

            if Options.ShowTracers.Value then
                if not data.Tracer then data.Tracer = Drawing.new("Line"); data.Tracer.Thickness = 1.5 end
                data.Tracer.Visible = true; data.Tracer.Color = color; data.Tracer.From = Vector2.new(Camera.ViewportSize.X / 2, Camera.ViewportSize.Y); data.Tracer.To = Vector2.new(screenPos.X, screenPos.Y)
            else
                if data.Tracer then data.Tracer.Visible = false end
            end
        else
...
Tabs.Settings:AddDropdown("ThemeDropdown", { Title = "Change Theme", Values = {"Dark", "Light", "Darker", "Aqua", "Amethyst", "Rose", "Liquid Glass"}, Default = "Dark", Callback = function(Value)
    if Value == "Liquid Glass" then pcall(function() Fluent:SetTheme("Dark"); Window:ToggleAcrylic(true) end)
    else pcall(function() Fluent:SetTheme(Value); Window:ToggleAcrylic(false) end) end
end})
...
Tabs.Settings:AddButton({ Title = "Unload Script", Callback = function()
    ...
    if Options.AutoWalk150 then Options.AutoWalk150:SetValue(false) end
